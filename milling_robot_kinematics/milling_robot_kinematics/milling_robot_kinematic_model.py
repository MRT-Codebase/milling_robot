...
class Robot:
    ...
    def trajectory(self, points, resolution, duration, history=0, draw_path=False):
        trajectory = self.interpolate(points, resolution)
        ...
        for i, point in enumerate(trajectory):
            ...
            hist = go.Scatter3d(
                x=trajectory[max(0, i - history):i, 0],
                y=trajectory[max(0, i - history):i, 1],
                z=trajectory[max(0, i - history):i, 2],
                mode='lines',
                name="Trajectory",
                line=dict(color= [f'rgba(0, 0, 0, {i})' for i in np.linspace(0.0, 0.5, i - max(0, i - history))])
            )
            trajectory_frames.append(go.Frame(data=frame_data+[hist]))
            ...
            layout=go.Layout(
                ...
                updatemenus=[dict(
                    ...
                    buttons=[dict(label="Play",
                                method="animate",
                                args=[None, {'frame': {'duration': duration*1000/len(trajectory_frames)}}])])]
            ),
